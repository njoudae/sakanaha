...
import { approximateCoordinates } from "@saknaha/utils/directions";
...
export default function RoommateCreatePage({
  ...
  const [region, setRegion] = useState(regionForCity(initialCity || user.city || "أبها"));
  ...
  function submit(event: FormEvent) {
    ...
    const approximateLocation = selectedUniversity
      ? approximateCoordinates({ lat: selectedUniversity.lat, lng: selectedUniversity.lng })
      : null;
    const property: Property = {
      ...
      universityNearby: selectedUniversity?.name ?? universityNearby.trim(),
      googleMapsUrl: "",
    ...
    addRoommateRequest({
      ...
      userType: user.role,
      age: 22,
      organization: selectedUniversity?.name ?? universityNearby.trim(),
      ...
      approximateLat: approximateLocation?.lat,
      approximateLng: approximateLocation?.lng,
      publicationStatus: "pending_review",
      submittedAt: now,
    });
    window.alert("تم إرسال بطاقة شريكة السكن لمراجعة الإدارة. الحالة: بانتظار المراجعة.");
    onDone();
  }
      ...
      >
        ...
        <label>
          ...
          <select
            ...
            onChange={(event) => {
              ...
              setCity(citiesForRegion(nextRegion)[0] ?? "");
        ...
        <label>
          ...
          >
            ...
            {citiesForRegion(region).map((cityName) => (
        ...
        <label>
          <span className="label">معلم قريب (اختياري)</span>
          <input
            className="field"
            value={landmark}
            onChange={(event) => setLandmark(event.target.value)}
            placeholder="مستشفى، شركة، مركز تسوق أو مسجد"
          />
        </label>
        ...
        <UniversityReferenceSelector
          ...
          onChange={(university) => {
            ...
            setUniversityNearby(university?.name ?? "");
          }}
          city={city}
        />
        <label>
